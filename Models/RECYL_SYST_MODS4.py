...
## REACTOR CLASS 
class CSTR():
    def __init__(self,T,F,C0,k0,Ea,Cinit,ρ,Cp,H,Tin,c,V,rev_ratio=rev_ratio):
    ...
    def MandE_bal(self,m_bal,q_bal):
        for i in range(self.T.shape[0]):
            ...
            self.Cf[i,:]=self.soln
            self.soln=q_bal(float(self.T[i, 0]),float(self.F[i, 0]),self.Cf[i, :],self.k,self.p[i],self.Cp[i],self.H,self.Tin[i],self.V)
...
class CSTR_REF():
    ...
    def MandE_bal(self,m_bal,q_bal):
        ...
        self.soln=q_bal(np.hstack([self.T1,self.T2]),self.F[:,0],self.p,self.Cp,self.H,self.Tin,self.V,self.thetam,self.rxt_num)
...
def Rxtr_Recycle(FR, TR, R_Frac, Cdist = np.ones(3),  rxn_reg = False):
    ...
    while all(err > 1e-6):
        ...
        for i in range(TR.shape[0]):
            ...
            CpR1[i]=MixR1.Cpmix
            FinR1[i]=MixR1.Fout
        ReacR1 = CSTR(TR[:, 0], FinR1, C0R1, k0R, EaR, Cinit3, ρR1, CpR1, HR, TinR1, c3, VR[0])
        ...
        for i in range(TR.shape[0]):
            ...
            CpR2[i]=MixR2.Cpmix
            FinR2[i]=MixR2.Fout
        ReacR2 = CSTR(TR[:, 1], FinR2, C0R2, k0R, EaR, Cinit3, ρR2, CpR2, HR, TinR2, c3, VR[1])
...
def RXTR_REG():
    ...
    def heat_bal_reg_recycle(T,F,p,Cp,H,Tin,V,theta,rxt_num):
        ...
        ToH2O=np.ones(T1.shape)
        for i in range(T1.shape[0]):
            ToH2O[i]=min(T1[i]-10,423) #313
    ...
    def Rxtr_Recycle_Ref(FR, TR, R_Frac, theta, Cdist = np.ones(3)):
        ...
        while all(err > 1e-6):
            ...
            for i in range(TR.shape[0]):
                ...
                pmix2=np.array([ReacR1_REF.p[i], ρR[1]])
                ...
                CpR2[i]=MixR2.Cpmix
                FinR2[i]=MixR2.Fout
            ReacR2_REF = CSTR_REF(TR[:, 0], TR[:, 1], FinR2, C0R2, ρR2, CpR2, HR, TinR2, c3, VR[1],
                                  theta[4:], rxt_num=2)
